...
function updateAutoHeight(speed) {
	...
	if (swiper.params.slidesPerView !== "auto" && swiper.params.slidesPerView > 1) if (swiper.params.centeredSlides) (swiper.visibleSlides || dom_default([])).each((slide) => {
...
function loopCreate() {
	...
	slides.each((el, index) => {
		dom_default(el).attr("data-swiper-slide-index", index);
	});
...
function onTouchStart(event) {
	...
	if (swipingClassHasValue && e.target && e.target.shadowRoot && eventPath) $targetEl = dom_default(eventPath[0]);
...
function onTouchMove(event) {
	...
	if (!swiper.allowTouchMove) {
		if (!dom_default(e.target).is(data.focusableElements)) swiper.allowClick = false;
		if (data.isTouched) {
...
function EffectCoverflow({ swiper, extendParams, on }) {
	...
	const setTranslate = () => {
		...
		for (let i = 0, length = slides.length; i < length; i += 1) {
			...
			const centerOffset = (center - $slideEl[0].swiperSlideOffset - slideSize / 2) / slideSize;
...
//# sourceMappingURL=swiper.esm-hUadtYLY.js.map