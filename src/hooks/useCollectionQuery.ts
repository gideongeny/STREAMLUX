...
export const useTMDBCollectionQuery = (
  ...
  useEffect(() => {
    const fetchData = async () => {
      // Create cache key parameters to check for immediate data
      let targetCountries: string[] = [];
      if (region) {
        switch (region.toLowerCase()) {
          case "africa": targetCountries = ["NG", "KE", "ZA", "GH", "TZ", "UG", "ET"]; break;
          case "asia": targetCountries = ["KR", "JP", "CN", "IN", "PH", "TH", "VN", "ID"]; break;
          case "latin": targetCountries = ["MX", "BR", "AR", "CO", "CL", "PE"]; break;
          case "middleeast": targetCountries = ["TR", "EG", "SA", "AE", "IR", "IL"]; break;
          ...
          case "china": targetCountries = ["CN"]; break;
          case "india": targetCountries = ["IN"]; break;
          case "philippines": targetCountries = ["PH"]; break;
          case "thailand": targetCountries = ["TH"]; break;
          case "turkey": targetCountries = ["TR"]; break;
          case "kenya": targetCountries = ["KE"]; break;
          case "south africa": targetCountries = ["ZA"]; break;
        }
      }
      ...
      try {
        // Optimization: If we have data, don't show full loading spinner
        // We still fetch to ensure freshness but the user sees results immediately
        if (data.length === 0) setIsLoading(true);
        
        setError(null);

        // FETCH: Use authoritative explore service and get multiple pages for 50-item support
        const fetchPage = async (page: number) => {
          return mediaType === "movie"
            ? await getExploreMovie(page, exploreConfig)
            : await getExploreTV(page, exploreConfig);
        };
        ...
        let results = [
          ...(Array.isArray(res1?.results) ? res1.results : []),
          ...(Array.isArray(res2?.results) ? res2.results : []),
          ...(Array.isArray(res3?.results) ? res3.results : [])
        ];
        ...
        if (filteredResults.length === 0) {
          ...
          filteredResults = (fallback?.results || []).map(item => ({ ...item, media_type: mediaType } as Item)).slice(0, 20);
        }
        ...
        const fallback = mediaType === "movie" ? await getExploreMovie(1, {}) : await getExploreTV(1, {});
        setData((fallback?.results || []).map(item => ({ ...item, media_type: mediaType } as Item)).slice(0, 20));
      } finally {
        setIsLoading(false);
      }
    };
