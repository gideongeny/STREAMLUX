...
import { hasHomeContent } from "../shared/gatewayResponse";
  ...
  const { data, isLoading, isError, error } = useQuery<HomeFilms, Error>(
    ...
    async () => {
      try {
        ...
        if (hasHomeContent(result as Record<string, unknown>)) {
          safeStorage.set(cacheKey, JSON.stringify(result));
          return result;
        }
        if (initialData && hasHomeContent(initialData as Record<string, unknown>)) {
          return initialData;
        }
        return result;
      } catch (err: unknown) {
