...
class APICache {
  ...
  private readonly DEFAULT_TTL = 30 * 60 * 1000; // 30 min — reduces TMDB gateway hits at scale
  ...
  /** Return last cached payload even if TTL expired (used when API quota is hit). */
  getStale(url: string, params?: any, maxAgeMs = 7 * 24 * 60 * 60 * 1000): any | null {
    const key = this.getCacheKey(url, params);
    const entry = this.cache.get(key);
    if (!entry) return null;
