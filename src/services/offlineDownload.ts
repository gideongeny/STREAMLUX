...
class OfflineDownloadService {
    ...
    /**
     * Sync a download item to Firestore so the native app can observe it.
     * Only updates Firestore when status changes or progress jumps by ≥5%.
     */
    private async syncToFirestore(item: DownloadItem): Promise<void> {
        const user = auth.currentUser;
        if (!user) return;

        const lastProgress = this.lastSyncedProgress.get(item.id) ?? -1;
        const progressDelta = Math.abs(item.progress - lastProgress);

        // Throttle: only sync on status change OR every 5% progress
        if (item.status === lastProgress.toString() && progressDelta < 5) return;

        try {
            const docRef = doc(db, 'users', user.uid, 'downloads', item.id);
            if (item.status === 'failed') {
                // Remove failed downloads from cloud after 1 hour (let native app handle cleanup)
                await setDoc(docRef, {
                    id: item.id,
                    title: item.title,
                    type: item.type,
            ...
            this.lastSyncedProgress.set(item.id, item.progress);
        } catch (e) {
            logger.error('Firestore sync failed:', e);
        }
    }

    /**
     * Remove a download from Firestore (on cancel/clear).
     */
    private async removeFromFirestore(itemId: string): Promise<void> {
        const user = auth.currentUser;
        if (!user) return;
        try {
            await deleteDoc(doc(db, 'users', user.uid, 'downloads', itemId));
        } catch (e) {
            logger.error('Firestore delete failed:', e);
        }
    }
    ...
    async addToQueue(item: Omit<DownloadItem, 'progress' | 'status' | 'downloadedBytes' | 'totalBytes' | 'addedAt'>): Promise<void> {
        ...
        this.saveDownloads();
        this.syncToFirestore(downloadItem); // push 'queued' status to cloud
    ...
    private async startDownload(itemId: string): Promise<void> {
        ...
        this.saveDownloads();
        this.syncToFirestore(item); // push 'downloading' status to cloud
        ...
        try {
            ...
            if (result.path) {
                ...
                this.syncToFirestore(item); // push 'completed' status to cloud
            ...
            item.status = 'failed';
            this.syncToFirestore(item); // push 'failed' status to cloud
        } finally {
    ...
    pauseDownload(itemId: string): void {
        ...
        if (item && item.status === 'downloading') {
            ...
            this.activeDownloads.delete(itemId);
            this.saveDownloads();
            this.syncToFirestore(item); // push 'paused' status to cloud
        }
    }
    ...
    async cancelDownload(itemId: string): Promise<void> {
        ...
        this.activeDownloads.delete(itemId);
        this.lastSyncedProgress.delete(itemId);
        this.saveDownloads();
        this.removeFromFirestore(itemId); // remove from cloud
