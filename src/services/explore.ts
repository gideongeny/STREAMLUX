...
export const getExploreMovie: (
  ...
  const hasSpecificFilter = (config as any).with_genres || (config as any).region || (config as any).with_origin_country;
  ...
  const fetchPromises = [
    ...
    // Only add popular movies as filler when no specific filter is set
    ...(hasSpecificFilter ? [] : [
...
export const getExploreTV: (
  ...
  const hasSpecificFilter2 = (config as any).with_genres || (config as any).region || (config as any).with_origin_country;
