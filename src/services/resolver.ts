...
// Known public Consumet instances for raw stream extraction
...
export class ResolverService {
    ...
    async resolveSources(
        ...
        // 0. Raw API Extractors (Priority: Maximum)
        // Consumet API (FlixHQ / Zoro) provides direct .m3u8 links
        try {
            const rawStreams = await this.getConsumetStreams(mediaType, id.toString(), season, episode);
            rawStreams.forEach(s => {
                s.priority = 45;
                sources.push(s);
            });
        } catch (e) {
            console.warn("Consumet raw extraction failed/skipped", e);
        }
        ...
        sources.push(
            ...
            },
            {
                name: "VidNest",
                url: mediaType === "movie"
                    ? `https://vidnest.fun/movie/${tmdbId}?servericon=show&bottomcaption=true`
                    : `https://vidnest.fun/tv/${tmdbId}/${season || 1}/${episode || 1}?servericon=show`,
                quality: "1080p", speed: "fast", status: "active", type: "embed", priority: 1
            },
            {
                name: "TouStream",
                url: mediaType === "movie"
                    ? `https://toustream.xyz/embed/movie/${tmdbId}`
                    : `https://toustream.xyz/embed/tv/${tmdbId}/${season || 1}/${episode || 1}`,
                quality: "1080p", speed: "fast", status: "active", type: "embed", priority: 5
            },
            {
                name: "AnimeHub",
                url: mediaType === "movie"
                    ? `https://dropfile.cc/player/movie/${tmdbId}`
                    : `https://dropfile.cc/player/tv/${tmdbId}/${season || 1}/${episode || 1}?audio=sub&lang=en`,
            ...
            },
            {
                ...
                url: mediaType === "movie"
                    ...
                    : `https://player.embed-api.stream/?id=${tmdbId}&s=${season || 1}&e=${episode || 1}`,
                quality: "1080p", speed: "fast", status: "active", type: "embed", priority: 6
            },
            {
                name: "Miruro",
                url: mediaType === "movie"
                    ? `https://indraembed.netlify.app/movie/${tmdbId}`
                    : `https://indraembed.netlify.app/tv/${tmdbId}/${season || 1}/${episode || 1}`,
                quality: "1080p", speed: "fast", status: "active", type: "embed", priority: 6
            },
            {
                name: "MeowTV",
                url: mediaType === "movie"
                    ? `https://meowtv.vercel.app/watch/${tmdbId}`
                    : `https://meowtv.vercel.app/watch/${tmdbId}?season=${season || 1}&ep=${episode || 1}`,
                quality: "1080p", speed: "fast", status: "active", type: "embed", priority: 6
            },
            {
                name: "VidSrc.vip",
        ...
        // VidKing first, then embeds by priority, then direct/HLS fallbacks last
        sources.sort((a, b) => {
            if (a.name === 'VidKing') return -1;
            if (b.name === 'VidKing') return 1;
            const aIsEmbed = a.type === 'embed' ? 0 : 1;
            const bIsEmbed = b.type === 'embed' ? 0 : 1;
            if (aIsEmbed !== bIsEmbed) return aIsEmbed - bIsEmbed;
            return a.priority - b.priority;
        });

        // Append direct HLS fallback at the end (often unavailable)
        sources.push({
            name: "CineSu",
            url: mediaType === "movie"
                ? `https://cine.su/v1/stream/master/movie/${tmdbId}.m3u8`
                : `https://cine.su/v1/stream/master/tv/${tmdbId}/${season || 1}/${episode || 1}.m3u8`,
            quality: "4K/1080p", speed: "fast", status: "active", type: "hls", priority: 99
        });
    ...
    /**
     * Fetch raw .m3u8 streams using Consumet API instances
     */
    private async getConsumetStreams(
        mediaType: "movie" | "tv",
        tmdbId: string,
        season?: number,
        episode?: number
    ): Promise<ResolvedSource[]> {
        const rawSources: ResolvedSource[] = [];
        
        // Loop through instances until one succeeds
        for (const instance of CONSUMET_INSTANCES) {
            try {
                const controller = new AbortController();
                const timeoutId = setTimeout(() => controller.abort(), 6000); // 6s timeout per instance

                // Step 1: Get media info and episode ID from TMDB ID
                const infoUrl = `${instance}/meta/tmdb/info/${tmdbId}?type=${mediaType}`;
                ...
                } else if (mediaType === "tv" && infoData.episodes) {
                    // Find the matching season and episode
                    const s = season || 1;
                    const e = episode || 1;
                    const ep = infoData.episodes.find((ep: any) => ep.season === s && ep.number === e);
                    if (ep) {
                        episodeId = ep.id;
                    }
                }
                ...
                // Step 2: Fetch the actual raw streaming links using the episode ID
                const watchUrl = `${instance}/meta/tmdb/watch/${episodeId}?id=${tmdbId}`;
                ...
                if (watchData.sources && Array.isArray(watchData.sources)) {
                    // Extract highest quality sources first
                    const sortedSources = watchData.sources.sort((a: any, b: any) => {
                    ...
                    sortedSources.forEach((src: any, idx: number) => {
                        if (src.url && (src.url.includes('.m3u8') || src.url.includes('.mp4'))) {
                            rawSources.push({
                                name: `StreamLux Raw (${src.quality || 'Auto'})`,
                                url: src.url,
                                quality: src.quality || "HD",
                                speed: "fast",
                                status: "checking",
                                type: src.url.includes('.m3u8') ? 'hls' : 'direct',
                                priority: 90 + idx
                            });
                        }
                    });
                    
                    // If we successfully got sources from this instance, don't query the other instances
                ...
                console.warn(`Consumet instance ${instance} failed`);
            }
        }
