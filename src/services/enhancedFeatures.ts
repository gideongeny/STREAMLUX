...
export const getSmartRecommendations = async (
  ...
  try {
    if (watchedItems.length === 0) {
      // If no history, return trending/popular content from multiple sources
      const [movies, tv, trending] = await Promise.all([
        getFZPopular("movie", 1),
        getFZPopular("tv", 1),
        axios.get("/trending/all/day", { params: { page: 1 } }).then(r => r.data.results || [])
      ]);
      const combined = [...movies, ...tv, ...trending].slice(0, limit);
