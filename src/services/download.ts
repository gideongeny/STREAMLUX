 ...
 */
const FIREBASE_ORIGIN = "https://streamlux-67a84.web.app";
...
export class DownloadService {
  ...
  private generateVideoSources(
    ...
    if (mediaType === "movie") {
      ...
      return [
        ...
        `${EMBED_ALTERNATIVES.APIMDB}/movie/${imdb}`,
  ...
  smartRedirect(info: DownloadInfo): void {
    ...
    if (Capacitor.isNativePlatform()) {
      Browser.open({ url: dlUrl }).catch((err) => {
        console.error('[CapacitorBrowser] Error opening URL:', err);
