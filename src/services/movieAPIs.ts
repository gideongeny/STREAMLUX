 ...
 */
const strictBrandItems = (items: Item[], mediaType: "movie" | "tv"): Item[] =>
  items
...
const discoverBrandPages = async (
  ...
  for (let p = 1; p <= pages; p++) {
    const batch = await discoverBrand({ ...base, page: p });
    merged.push(...strictBrandItems(batch, mediaType));
...
export const getTMDBByBrand = async (
  ...
  if (id === "007") {
    try {
      ...
      const parts: Item[] = strictBrandItems(
        ((col.data?.parts || []) as Item[]),
        "movie"
      );
      parts.sort((a, b) =>
  ...
  const companyBrands: Record<string, {
    ...
    starwars: { keywords: "803", minVotes: 0 },
  ...
  if (mapping.minVotes != null) baseParams["vote_count.gte"] = mapping.minVotes;

  try {
    if (id === "starwars") {
      const movies = await discoverBrandPages(
        { ...baseParams, endpoint: "/discover/movie", with_keywords: "803" },
        "movie",
        2
      );
      const tv = await discoverBrandPages(
        {
          endpoint: "/discover/tv",
          with_keywords: "803",
          sort_by: "popularity.desc",
      ...
      const pick = resolvedType === "tv" ? tv : movies;
      return pick.length > 0 ? pick : [...movies, ...tv].slice(0, 40);
    }

    return discoverBrandPages(baseParams, resolvedType, page === 1 ? 2 : 1);
  } catch (error) {
