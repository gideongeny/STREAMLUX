...
import { setTrack, setQueue, pause } from '../../store/slice/musicSlice';
...
const GenreRow: React.FC<{ genre: string; isPlaying: boolean; currentId?: string; onPlay: (track: MusicTrack, list: MusicTrack[]) => void }> = ({ genre, isPlaying, currentId, onPlay }) => {
  ...
  useEffect(() => {
    if (inView) {
      ...
      musicService.search(`${genre} Hits playlist 2024`).then(res => setTracks(res.slice(0, 20)));
    }
  }, [inView, genre]);
...
const MusicHub: React.FC<MusicHubProps> = ({ isEmbed, searchQuery = "" }) => {
  ...
  useEffect(() => {
    ...
    // Pause music when navigating away from the Music Hub
    return () => {
      dispatch(pause());
    };
  }, []);
    ...
    <div className={`${isEmbed ? '' : 'min-h-screen pt-24'} bg-[#050505] text-white pb-48 px-4 md:px-10 overflow-hidden`}>
      <div className="max-w-[1600px] mx-auto">
        ...
        <AnimatePresence mode="wait">
          {(localSearchQuery || searchQuery) && searchResults.length > 0 ? (
            <motion.section 
                key="search"
        ...
        >
            ...
            <div className="mt-6 px-3">
                ...
                <div className="flex items-center justify-between mt-2">
                    ...
                    {(track.source === 'saavn' || track.source === 'youtube') && (
