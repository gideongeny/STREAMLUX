...
import { isYouTubeChannel, resolveYouTubeId } from '../../utils/youtubeLiveTV';
import LiveTVYouTubePlayer from './LiveTVYouTubePlayer';
import { FiChevronLeft, FiMenu, FiX, FiInfo, FiZap, FiMaximize2, FiMinimize2 } from 'react-icons/fi';
...
const TVWatchPage: React.FC = () => {
  ...
  useEffect(() => {
    const resolveChannel = async () => {
      // 1. Resolve channel from state or static list
      let found = location.state?.channel || ALL_TV_CHANNELS.find(c => c.id === channelId);
      
      // 2. If not found, try fetching from Firestore
      if (!found && channelId) {
        try {
          const dbChannel = await liveTVService.getChannelById(channelId);
          if (dbChannel) found = dbChannel;
        } catch (err) {
          console.error("Failed to resolve channel from DB", err);
        }
      }
      ...
      if (found) {
        ...
        // Only navigate away if we've explicitly failed to find it
        if (channelId) {
           navigate('/live-tv'); 
        }
      }
    ...
    resolveChannel();
  }, [channelId, location.state, navigate]);
  ...
  const youtubeId = isYouTubeChannel(currentChannel) ? resolveYouTubeId(currentChannel) : null;
  ...
  return (
    <div className="flex flex-col h-screen bg-black overflow-hidden select-none">
      ...
      <div className="h-20 bg-[#050505]/90 backdrop-blur-2xl border-b border-white/5 flex items-center justify-between px-6 z-50">
        ...
        <div className="flex items-center gap-4">
           <button 
                ...
                className="p-3 bg-white/5 border border-white/10 rounded-2xl hover:bg-white/10 transition-all text-gray-400 hover:text-primary"
      ...
      <div className="flex flex-grow overflow-hidden relative" ref={playerContainerRef}>
        {/* Main Player View */}
        <div className="flex-grow flex flex-col bg-black relative transition-all duration-500 overflow-hidden">
          <div className="flex-grow relative">
            {youtubeId ? (
              <LiveTVYouTubePlayer videoId={youtubeId} channelName={currentChannel.name} />
          ...
          {/* StreamSports99 Branding Footer */}
          <div className="bg-[#1a1a2e] py-3 px-6 border-t border-white/5 flex items-center justify-center gap-4 animate-in fade-in slide-in-from-bottom-2 duration-700">
            <a 
              href="https://StreamSports99.website" 
              target="_blank" 
              rel="noopener noreferrer"
              className="text-[10px] font-black uppercase tracking-[0.3em] text-[#22c55e] hover:text-white transition-all transform hover:scale-110 flex items-center gap-2"
            >
