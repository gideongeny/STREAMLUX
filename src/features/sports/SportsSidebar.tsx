...
const SportsSidebar: React.FC<SportsSidebarProps> = ({ 
  ...
  const handleMatchClick = (match: SportMatch) => {
    ...
    const sport = encodeURIComponent(match.sport || match.sportsCategory || "soccer");
    navigate(`/sports/arena/${match.id}?home=${home}&away=${away}&sport=${sport}`, { state: { streamUrl: match.link } });
  };
