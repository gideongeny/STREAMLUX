          ...
          <>
            ...
            {detail && (
              <>
                ...
                {detail.media_type === "movie" && (detail as DetailMovie).budget > 0 && (
                  <p>Budget: <span className="text-primary font-bold">${((detail as DetailMovie).budget / 1000000).toFixed(1)}M</span></p>
                )}
                {detail.media_type === "movie" && (detail as DetailMovie).revenue > 0 && (
                  <p>Revenue: <span className="text-green-500 font-bold">${((detail as DetailMovie).revenue / 1000000).toFixed(1)}M</span></p>
                )}
                {detail.media_type === "tv" && (detail as DetailTV).networks && (
                  <p>
                    Network:
                    {(detail as DetailTV).networks.map(
                      (net, index) =>
                        `${index ? ", " : ""} ${net.name}`
                    )}
                  </p>
