...
import Hls from '@swarmcloud/hls';
...
const CLEAN_SOURCES = [
    'vidking.net', 'vidlink.pro', 'vidsrc.me', 'vidsrc.to', 'vidsrc.vip', 'vidsrc.pro', 'vidsrc.xyz',
    'embed.su', 'superembed.stream', '2embed.org', 'autoembed.cc', 'moviesapi.club',
    'smashystream.com', 'multiembed.mov', 'vidfast.pro',
];
...
const StreamLuxPlayer: React.FC<VideoPlayerProps> = ({
    ...
    const [p2pStatus, setP2pStatus] = useState<{ active: boolean, peers: number }>({ active: false, peers: 0 });
    ...
    useEffect(() => {
        ...
        const direct =
            currentSource.type === 'embed'
                ? false
                : currentSource.type === 'direct' || currentSource.type === 'hls' || isDirectVideoUrl(currentSource.url);
        setIsDirect(direct);
        setVideoError(false);
        setIsLoading(true);
        ...
        if (direct && currentSource.url.includes('.m3u8') && videoRef.current) {
            ...
            if (Hls.isSupported()) {
                ...
                hls.on(Hls.Events.P2P_STATS, (data: any) => {
                    setP2pStatus({ active: true, peers: data.totalPeers || 0 });
                });
                    ...
                    if (data.fatal) {
                        switch (data.type) {
                            case Hls.ErrorTypes.NETWORK_ERROR:
                                // If manifest load fails or we retry too many times, fail and fallback immediately
                                if (data.details === Hls.ErrorDetails.MANIFEST_LOAD_ERROR || hlsRetryCount >= 1) {
                                    handleVideoError();
                                } else {
                                    hlsRetryCount++;
                                    hls.startLoad();
    ...
    useEffect(() => {
        ...
        if (selectedSourceIndex !== undefined && selectedSourceIndex >= 0) {
            setCurrentIndex(selectedSourceIndex);
        }
        setVideoError(false);
        setIsLoading(selectedSourceIndex !== undefined && selectedSourceIndex >= 0);
        setAudioTracks([]);
        setActiveAudio('');
        // eslint-disable-next-line react-hooks/exhaustive-deps
    }, [normalizedSources[0]?.url, selectedSourceIndex]);
    ...
    const handleVideoError = () => { 
        ...
        setVideoError(true); 
        setIsLoading(false); 
        toast.error(t("Source unavailable. Please select another server."), { position: "top-center", autoClose: 4000 });
        if (onError) onError(); 
    };
    ...
    const getAutoplayUrl = (url: string) => {
        ...
        if (url.includes('youtube.com') || url.includes('youtu.be') || url.includes('youtube-nocookie.com')) {
            ...
            let finalUrl = `${url}${separator}autoplay=1&enablejsapi=1&origin=${encodeURIComponent(origin)}&rel=0&iv_load_policy=3`;
            return finalUrl.replace('youtube.com', 'youtube-nocookie.com');
        }
        ...
        if (url.includes('vidsrc.') || url.includes('vidlink') || url.includes('autoembed') || url.includes('moviesapi')) {
    ...
    return (
        ...
        >
            ...
            {/* P2P Mesh Badge */}
            {p2pStatus.active && isDirect && !isLoading && !videoError && (
                <motion.div 
                    initial={{ opacity: 0, x: 20 }}
                    animate={{ opacity: 1, x: 0 }}
                    className="absolute top-4 right-4 z-[100] flex items-center gap-2 bg-black/40 backdrop-blur-md border border-white/10 px-3 py-1.5 rounded-full"
                >
                    ...
                    <span className="text-[10px] font-black text-white uppercase tracking-tighter">
                        P2P Mesh: {p2pStatus.peers} Peers
                    </span>
                </motion.div>
            )}
