...
const CinematicMoments: FC = () => {
    ...
    const handleOpenStory = async (index: number) => {
        ...
        try {
            ...
            // Only open the story if we have a valid trailer — never fall back to unrelated content
            if (trailer?.key) {
                setActiveStory({ ...item, videoId: trailer.key });
            } else {
                // Show the story card without video (poster-only mode)
                setActiveStory({ ...item, videoId: null });
            }
        } catch (err) {
            setActiveStory({ ...item, videoId: null });
        }
                        ...
                        <div className="relative w-full max-w-[450px] aspect-[9/16] rounded-3xl overflow-hidden border border-white/10 shadow-2xl bg-black">
                            ...
                            {activeStory.videoId ? (
                                <iframe
                                    ...
                                    title={`${activeStory.title || activeStory.name} trailer`}
                                />
                            ) : (
                                <div className="w-full h-full relative">
                                    <img
                                        src={resizeImage(activeStory.backdrop_path || activeStory.poster_path || "", "w780")}
                                        alt={activeStory.title || activeStory.name}
                                    ...
                                    <p className="absolute bottom-24 left-0 right-0 text-center text-white/60 text-xs font-medium px-6">No trailer available</p>
                                </div>
                            )}
