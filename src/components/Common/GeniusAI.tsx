...
import { setGeniusOpen } from '../../store/slice/uiSlice';
...
const GeniusAI: React.FC = () => {
  ...
  const isOpen = useAppSelector((state) => state.ui.isGeniusOpen);
  const setIsOpen = (open: boolean) => dispatch(setGeniusOpen(open));
  ...
  const handleSend = async (textOverride?: string) => {
    ...
    // Rate Limiting Check
    const now = Date.now();
    const recentRequests = requestTimestamps.filter(t => now - t < RATE_LIMIT_WINDOW_MS);
    ...
    // Sanitize and wrap input to prevent prompt injection
    const sanitizedUserMessage = userMessage.replace(/<[^>]*>?/gm, '');
    const wrappedUserMessage = `<user_input> ${sanitizedUserMessage} </user_input>`;
    
    const newHistory = [...history, { role: 'user', content: wrappedUserMessage }];
