    ...
    <Link
      to={
        item.url && item.isExternal
          ? "#"
          : (item.media_type === "movie" || (!item.media_type && (item.title && !item.name && !(item as any).first_air_date)))
            ? `/movie/${item.id || item.youtubeId}`
            : (item.media_type === "tv" || (!item.media_type && ((item as any).first_air_date || (item.name && !item.title))))
              ? `/tv/${item.id}`
              : (item.media_type === "sports" || item.media_type === "sports_video")
                ? `/sports/${(item as any).leagueId || 'all'}/${item.id}/watch`
                : (item.media_type === "live-tv" || item.isLive)
                  ? `/live-tv/${item.id}`
                  : `/`
      }
      onClick={(e) => {
    ...
    >
      ...
      >
        ...
        >
          ...
          <p className="whitespace-nowrap overflow-hidden text-ellipsis text-[12px] font-semibold text-[#888] mt-2.5 text-center px-2 group-hover:text-white transition-all duration-500 tracking-tight">
            {item.title || item.name}
          </p>

          <div className="bg-black/60 backdrop-blur-[20px] px-2 py-1 rounded-lg absolute top-[6%] left-[6%] z-20 flex items-center gap-1 text-white text-[10px] font-bold border border-white/[0.08] shadow-glass-subtle">
          ...
          {item.vote_average > 8 && !isUnreleased && (
            <div className="bg-primary/15 backdrop-blur-[20px] border border-primary/25 text-primary px-2.5 py-1 rounded-lg absolute top-[6%] right-[5%] z-20 text-[9px] font-black uppercase tracking-wider flex items-center gap-1 shadow-cinema-glow">
              Top Hit
            </div>
          )}
        </motion.div>
