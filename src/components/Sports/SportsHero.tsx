...
import { MdPlayArrow, MdInfoOutline, MdNotificationsActive, MdClose } from "react-icons/md";
...
const SportsHero: FC<SportsHeroProps> = ({ featuredMatch, isLoading, getMatchLink }) => {
    ...
    // Live Data Polling
    useEffect(() => {
        if (!isLive || !featuredMatch?.id) return;

        const fetchData = async () => {
            const events = await getMatchEvents(featuredMatch.id);
            const stats = await getMatchStatistics(featuredMatch.id);

            setMatchEvents(events.map(e => ({
                id: `e-${e.time?.elapsed}-${e.team?.id}`,
                type: e.type?.toLowerCase() || 'attack',
                team: e.team?.name === featuredMatch.homeTeam ? 'home' : 'away',
                x: Math.random() * 80 + 10, // API doesn't usually provide X/Y for free, using dynamic placement
                y: Math.random() * 80 + 10,
                timestamp: Date.now()
            })));

            if (stats && stats.length > 0) {
                const homePoss = stats.find(s => s.team?.name === featuredMatch.homeTeam)?.statistics?.find((st: any) => st.type === "Ball Possession")?.value || "50%";
                const awayPoss = stats.find(s => s.team?.name === featuredMatch.awayTeam)?.statistics?.find((st: any) => st.type === "Ball Possession")?.value || "50%";
                setPossession({
                    home: parseInt(homePoss.toString()),
                    away: parseInt(awayPoss.toString())
                });
            }
        };
    ...
    return (
        <section className="relative w-full aspect-[21/9] min-h-[450px] md:min-h-[550px] rounded-[3rem] overflow-hidden group shadow-[0_50px_100px_-20px_rgba(0,0,0,0.5)] border border-white/10">
                ...
                <AnimatePresence mode="wait">
                    {!showVisualizer ? (
                        ...
                        >
                            ...
                            <div className="absolute inset-0 z-10">
                                <SportsHeroTrailer
                                    ...
                                    fallbackImageUrl={featuredMatch.banner || featuredMatch.thumb || featuredMatch.poster_path}
                                />
                            </div>
                        </motion.div>
                    ) : (
                        <motion.div
                            ...
                            className="absolute inset-0 z-10 bg-[#050505] flex items-center justify-center p-10"
                        >
                            <MatchVisualizer
                                homeTeam={featuredMatch.homeTeam || "Home"}
                                awayTeam={featuredMatch.awayTeam || "Away"}
                                events={matchEvents}
            ...
            {/* Content Overlay */}
            <div className={`absolute inset-0 flex flex-col justify-center px-8 md:px-20 z-30 transition-opacity duration-500 ${showVisualizer ? 'opacity-0 pointer-events-none' : 'opacity-100'}`}>
                <motion.div
                ...
                >
                    ...
                    <div className="flex flex-wrap items-center gap-4 pt-4">
                        <a
                            ...
                            className="flex items-center gap-3 px-10 py-5 bg-primary text-black rounded-[2rem] font-black text-lg transition-all duration-300 hover:scale-105 hover:bg-white shadow-[0_20px_40px_-10px_rgba(255,107,53,0.3)] group/btn"
                        >
                        ...
                        {(isLive && hasTeams) && (
                            <button
                                ...
                                className="flex items-center gap-3 px-8 py-5 bg-white/10 backdrop-blur-xl text-white rounded-[2rem] font-bold border border-white/10 transition-all duration-300 hover:bg-white/20"
                            >
