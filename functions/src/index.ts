...
const TMDB_CREDENTIALS = [
    {
        apiKey: (process.env.TMDB_API_KEY_1 || "d87dbd2496ba67b311d9012ed55bc3bb").trim(),
        bearer: (process.env.TMDB_BEARER_TOKEN_1 || "eyJhbGciOiJIUzI1NiJ9.eyJhdWQiOiJkODdkYmQyNDk2YmE2N2IzMTFkOTAxMmVkNTViYzNiYiIsIm5iZiI6MTc3OTUwMzM2Ni44MjYsInN1YiI6IjZhMTExMTA2NThjZGZhMjFmNGI1YTVjNSIsInNjb3BlcyI6WyJhcGlfcmVhZCJdLCJ2ZXJzaW9uIjoxfQ.tkiRuY9vJkjBVvrJZ-3dLdbj-XPr_m2XsJoJ7C1QWwQ").trim(),
    },
    {
        apiKey: (process.env.TMDB_API_KEY_2 || "be86af046da20f5bc823fe58fc7ff33e").trim(),
        bearer: (process.env.TMDB_BEARER_TOKEN_2 || "eyJhbGciOiJIUzI1NiJ9.eyJhdWQiOiJiZTg2YWYwNDZkYTIwZjViYzgyM2ZlNThmYzdmZjMzZSIsIm5iZiI6MTc3OTUwMzcxNC42MTc5OTk4LCJzdWIiOiI2YTExMTI2MjE3YzM2ZjNjMTBhZWI5ZDkiLCJzY29wZXMiOlsiYXBpX3JlYWQiXSwidmVyc2lvbiI6MX0.FMZGLJpLNhN942QAV4abXjpNPNtMru7uBovIasui9nc").trim(),
    },
    {
        apiKey: (process.env.TMDB_API_KEY || "7ea638a69773174284507081474e892d").trim(),
        bearer: (process.env.TMDB_BEARER_TOKEN || "eyJhbGciOiJIUzI1NiJ9.eyJhdWQiOiI3ZWE2MzhhNjk3NzMxNzQyODQ1MDcwODE0NzRlODkyZCIsIm5iZiI6MTc1NDgyNjU1Mi4zMTcsInN1YiI6IjY4OTg4NzM4NzczZjAxYzIzNDVkMGRlYSIsInNjb3BlcyI6WyJhcGlfcmVhZCJdLCJ2ZXJzaW9uIjoxfQ.dIHNX2XlReX3c8917Ug5Sw9QLf_SEkIIcakh4jCsOos").trim(),
    },
];
const TMDB_API_KEY = TMDB_CREDENTIALS[0].apiKey;

async function fetchTmdbWithFallback(endpoint: string, params: Record<string, unknown>) {
    const path = endpoint.startsWith("/") ? endpoint : `/${endpoint}`;
    let lastErr: any = null;
    for (const cred of TMDB_CREDENTIALS) {
        try {
            const response = await axios.get(`${TMDB_BASE_URL}${path}`, {
                params: { ...params, api_key: cred.apiKey },
                headers: {
...
export const gateway = functions
    ...
    .https.onRequest(async (req, res) => {
        ...
        const allowedOrigins = [
            ...
            'http://localhost:8080'  // Alt Capacitor Local
        ];
        ...
        try {
            ...
            if (rawPath === 'sports/live' || rawPath === 'sports/upcoming') {
                ...
                const mapEspnEvent = (event: any, endpoint: string) => {
                    ...
                    const venueImg = comp?.venue?.images?.[0]?.href;
                    const cover = venueImg || home?.team?.logo || away?.team?.logo;
                ...
                // CricHd auto-updated sports TV (https://github.com/abusaeeidx/CricHd-playlists-Auto-Update-permanent)
                if (wantLive) {
                    try {
                        const crichdRes = await axios.get(
                            'https://raw.githubusercontent.com/abusaeeidx/CricHd-playlists-Auto-Update-permanent/main/api.json',
                            { timeout: 12000 }
                        );
                        const channels = crichdRes.data;
                        if (Array.isArray(channels)) {
                            channels.slice(0, 40).forEach((ch: any, idx: number) => {
            ...
            // --- INNERTUBE (YouTube Music / Video — free, no Data API quota) ---
            if (rawPath === 'innertube' || rawPath.includes('innertube')) {
                ...
                const itUrl = `https://${host}/youtubei/v1${String(endpoint).startsWith('/') ? endpoint : '/' + endpoint}?key=${key}`;
                const clientVersion =
                ...
                res.status(200).json(itData);
                return;
            }

            // --- PIPED PROXY (server-side search/streams, fixes browser CORS) ---
            if (rawPath.startsWith('piped')) {
                ...
                for (const base of PIPED_INSTANCES) {
                    try {
                        const target = rawPath.includes('streams') && videoId
                            ? `${base}/streams/${videoId}`
                            : `${base}/search?q=${encodeURIComponent(q)}&filter=${encodeURIComponent(filter)}`;
                        const pRes = await fetch(target, { signal: AbortSignal.timeout(10000) });
            ...
            if (rawPath === 'tmdb' || rawPath.startsWith('proxy/tmdb') || rawPath.includes('tmdb') || req.query.endpoint) {
                ...
                try {
                    const response = await fetchTmdbWithFallback(
            ...
            if (rawPath.includes('proxy/external') || rawPath.includes('external')) {
                ...
                switch (provider) {
                    ...
                    case "sofascore":
                        targetUrl = `https://api.sofascore.com/api/v1${endpoint}`;
                        const sofaResponse = await axios.get(targetUrl, { params });
                        res.status(200).json(sofaResponse.data);
                        return;

                    case "watchfooty":
                        const wfUrl = req.query.url as string || req.body.url as string;
                        if (!wfUrl) {
                            res.status(400).json({ error: 'Missing WatchFooty target URL' });
                            return;
                        }
                        const wfResponse = await axios.get(wfUrl, {
                            headers: { ...headers, 'Referer': 'https://watchfooty.st/' }
                        });
                        res.status(200).json(wfResponse.data);
                        return;
                    ...
                    case "serpapi":
                        targetUrl = "https://serpapi.com/search";
                        const serpParams = { ...params, api_key: SERP_API_KEY, engine: "google_short_videos" };
                        const serpResponse = await axios.get(targetUrl, { params: serpParams });
                        res.status(200).json(serpResponse.data);
                        return;
...
export const notifyTrendingContent = functions.pubsub
    ...
    .onRun(async (context) => {
        ...
        const response = await fetchTmdbWithFallback("/trending/all/day", {});
